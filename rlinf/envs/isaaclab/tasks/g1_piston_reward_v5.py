...
"""Reward v5: v3 with a GEOMETRY-GROUNDED plunger press (threshold, hold, and gates).

v3 stays the scorer of record for the transport stages (reach, grasp, lift, plate), which
v5 leaves bit-for-bit identical. v5 redefines only the two plunger stages, because the v3
press predicate was measured to be physically unreachable without contact interpenetration:

* **The rod protrudes 25 mm above the barrel at rest** (rod top at local z 0.180, barrel
  top at 0.155; measured 0.021-0.025 m across replays). Pressing moves the barrel UP the
  rod, so at 25 mm of travel the rod top is flush with the barrel top, and the barrel's
  collision hull is a CONVEX HULL that fills the bore. Nothing can push the rod deeper
  than flush without penetrating that hull. v3's threshold was 0.70 x 40 mm = **28 mm**:
  3 mm past the ceiling. Every v3 press RL ever certified (28-32 mm) was a table press
  driven by the arm's full PD force into interpenetration (g1_piston_table_press_exploit).
* **A clean thumb press exists and reaches exactly the ceiling.** With the pipette held in
  the air, sweeping the thumb yaw (action dim 25) to its 1.3 rad limit brings the thumb
  intermediate link over the rod top (1.6 cm off-axis, dz 0.0) and depresses the plunger to
  24.5 mm, where it holds steadily (rod top 0.3 mm above the barrel top). Measured in
  runs_g1_piston/diag/press_mechanics (trial ``air_thumb_yaw_1.3``).
* **Inertial transients exist and must not count.** The one "press" in the demonstrations
  (ep46, 19.5-23.9 mm) is a snatch: the barrel is yanked up into the hand during the lift
  and the rod lags for ~6 control steps. A real press is SUSTAINED.

v5 therefore defines the press as: plunger depth > ``PRESS_DEPTH`` = 20 mm (50 % of
travel, 5 mm above flush, above the 8-15 mm incidental band of every non-snatch step) for
at least ``PRESS_SUSTAIN_STEPS`` = 15 consecutive control steps (0.3 s), while the FINGERS
...
Everything below the marker is v3's own text and code, unchanged except where marked v5.

---- original v3 documentation follows ----
...
PRESS_FRAC = 0.50
#: Absolute press threshold, metres. v5: 20 mm = 50 % of travel, 5 mm above the flush
#: ceiling (ROD_PROTRUSION) and above the 8-15 mm incidental band of every non-snatch
#: demonstration step. v3 used 0.70 (28 mm), which is 3 mm PAST the physical ceiling.
PRESS_DEPTH = PLUNGER_TRAVEL * PRESS_FRAC
#: Rod top above the barrel top at rest (local 0.180 - 0.155). A press of this depth puts
#: the rod flush with the barrel; deeper requires penetrating the barrel's convex hull.
ROD_PROTRUSION = 0.025
#: Consecutive control steps the depth must be held for a press to count (0.3 s at 50 Hz).
#: The demonstrations' inertial snatch transient lasts ~6 steps.
PRESS_SUSTAIN_STEPS = 15
#: Identifies this reward in run records. v3 (scorer of record for transport) is
#: "v3_review_fixed".
REWARD_VERSION = "v5_geometric_press"
...
class PistonTaskRewardV5:
    """v3 transport stages + geometry-grounded, sustained, lifted, finger-held press."""
    ...
    def step(self):
        ...
        if pays_press:
            ...
            if press > self._best_press:
                ...
                self._best_press = press
            self._max_press_held = max(self._max_press_held, press)
        self._max_press = max(self._max_press, press)
        # v5: sustained-depth counter. Inertial transients last ~6 steps; a press must
        # hold PRESS_DEPTH for PRESS_SUSTAIN_STEPS consecutive steps.
        if held_press_ok and press > PRESS_DEPTH:
            self._press_steps += 1
        else:
            self._press_steps = 0
