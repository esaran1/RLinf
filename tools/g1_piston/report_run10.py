...
"""Fill the run-10 section of verified_results/FINAL_REPORT.md from certified files.
...
rows = {}
for p in sorted(glob.glob(f"{D}/filter_ab/rl10_iter*_s?_v3_n25.json")):
    it = re.search(r"rl10_(iter\d+)_s(\d)", p)
    try: rows.setdefault(it.group(1), {})[int(it.group(2))] = m(p)
    except Exception: pass
train = json.load(open(f"{D}/rl10/run.json"))
sel = ""
if os.path.exists(f"{D}/chain_rl10.log"):
    for line in open(f"{D}/chain_rl10.log"):
        if "selected:" in line: sel = line.split("selected:")[-1].strip()
lines = ["## Run 10 (training reward v4: press pays only while lifted) — certified", "",
...
for it in sorted(rows, key=lambda s: int(s[4:])):
    for s_ in sorted(rows[it]):
        x = rows[it][s_]
        lines.append(f"| run 10 {it} | {s_} | {x['grasp_rate']:.2f} | {x['lift_rate']:.2f} | {x['plate_rate']:.2f} | {x['press_rate']:.2f} ({x['press_with_lift']}, {x['press_without_lift']}) | {x['mean_return']:.2f} |")
...
start = rep.index("## Run 10 (training reward v4)"); end = rep.index("## Evaluation caveats")
