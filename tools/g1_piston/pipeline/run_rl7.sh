#!/usr/bin/env bash
# Run 7 (residual arm, ResFiT), pre-registered in
...
OUTF=$D/rl7/run.json RUN_DIR=$D/rl7 \
  ...
  timeout 28800 $PY tools/g1_piston/train_sac.py > $D/rl7/train.log 2>&1
echo "rl7 exit=$?" >> $D/rl7/train.log
