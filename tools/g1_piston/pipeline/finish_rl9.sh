...
[ -f $D/rl9/grpo_ckpt_best.pt ] && score rl9_best $D/rl9/grpo_ckpt_best.pt
PASS=$($PY -c "
import json
try: d=json.load(open('$D/filter_ab/rl9_best_v3_n25.json')); m=d['modes']['deterministic']; print('1' if d.get('_status')=='OK' and m['grasp_rate']>=0.87 else '0')
except Exception: print('0')")
if [ "$PASS" = "1" ]; then
  CONDS=$($PY -c "
import json;pc=json.load(open('$D/filter_ab/rl9_best_v3_n25.json'))['modes']['deterministic']['per_condition']
...
print(','.join(str(r['condition']) for r in best))")
  mkdir -p $D/videos/rl9; wait_gpu
  OUTDIR=$D/videos/rl9 CKPT=$D/rl9/grpo_ckpt_best.pt CONDS=$CONDS MODE=deterministic TAG=rl9 REWARD_V3=1 RESET_SUITE_SEED=20260817 EP_CHUNKS=23 FPS=20 LOG_STEPS=1 timeout 9000 $PY $T/render_rollouts.py > $D/videos/rl9_render.log 2>&1
  echo "$(date +%H:%M:%S) rendered $CONDS exit=$?" >> $LOG
else echo "$(date +%H:%M:%S) run 9 best did not pass H4; no videos" >> $LOG; fi
# re-run the two measurements that OOM'd under another process on 2026-09-11
...
echo "$(date +%H:%M:%S) FINISH RL9 DONE" >> $LOG
