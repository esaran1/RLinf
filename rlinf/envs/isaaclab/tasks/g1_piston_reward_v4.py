...
"""TRAINING reward v4: v3 with the plunger terms gated on a HELD (lifted) pipette.

v3 remains the SCORER OF RECORD; v4 is a training objective. The only differences are the
two gates marked 'v4' below. Reason: g1_piston_table_press_exploit.json.
...
class PistonTaskRewardV4:
    ...
    def step(self):
        ...
        # v4: gate on HELD (grasped AND lifted above LIFT_H), not on grasp alone. Under v3,
        # critic-free RL found that pressing the plunger against the TABLE while grasping
        # -- never lifting -- earns the 12.0 stage bonus plus this dense term, for a return
        # of 15-18, more than a full transport (~14). Every certified press in runs 9b/9c
        # occurred with NO lift (g1_piston_table_press_exploit.json). The intended act is a
        # press with the pipette in the air, so that is what pays.
        ...
        fire("press", grasped and lift > LIFT_H and press > PRESS_DEPTH)
